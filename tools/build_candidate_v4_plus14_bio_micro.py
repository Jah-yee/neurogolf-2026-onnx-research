...
ANCHOR = ROOT / "submissions/candidate_v4_plus13_task366_onnx"
SOURCE = ROOT / "submissions/biohack44_superior_onnx"
OUTPUT = ROOT / "submissions/candidate_v4_plus14_bio_micro_onnx"
ZIP_PATH = ROOT / "submissions/submission_candidate_v4_plus14_bio_micro.zip"
MANIFEST = ROOT / "reports/candidate_v4_plus14_bio_micro_manifest.csv"
SUMMARY = ROOT / "reports/candidate_v4_plus14_bio_micro_build.json"

CHANGED_TASKS = ["task145", "task153", "task248", "task357"]
...
def main() -> None:
    ...
    for task in CHANGED_TASKS:
        shutil.copy2(SOURCE / f"{task}.onnx", args.output / f"{task}.onnx")
    ...
    if changed != CHANGED_TASKS:
        raise RuntimeError(f"changed task mismatch: actual={changed}, expected={CHANGED_TASKS}")
