...
ANCHOR = ROOT / "submissions/candidate_v4_plus15_bio_micro2_onnx"
SOURCE_TASK = Path("/tmp/ng_seddik_precision/submission/task001.onnx")
OUTPUT = ROOT / "submissions/candidate_v4_plus16_seddik_t001_onnx"
ZIP_PATH = ROOT / "submissions/submission_candidate_v4_plus16_seddik_t001.zip"
MANIFEST = ROOT / "reports/candidate_v4_plus16_seddik_t001_manifest.csv"
SUMMARY = ROOT / "reports/candidate_v4_plus16_seddik_t001_build.json"
CHANGED_TASK = "task001"
...
def main() -> None:
    args = parse_args()
    if len(list(ANCHOR.glob("task*.onnx"))) != 400:
        raise RuntimeError(f"bad anchor dir: {ANCHOR}")
    if not SOURCE_TASK.exists():
        raise RuntimeError(f"missing source task: {SOURCE_TASK}")

    if args.output.exists():
        shutil.rmtree(args.output)
    shutil.copytree(ANCHOR, args.output)
    shutil.copy2(SOURCE_TASK, args.output / "task001.onnx")
