"""DSL v1 depth-≤2 search runner.

For each task in the sample:
  1. Run `search_program` (depth-1 then depth-2 over the v1 primitive bank).
  2. If a candidate is found, compile to ONNX, save under
     `submissions/handbuilds/dsl_v1/dsl_task<id>.onnx`, and run
     `isolated_task_eval.py` to record cost + train/test/arc-gen pass.
  3. Compare against the v4_plus4 baseline cost; flag as a swap candidate if
     `dsl_cost <= baseline_cost` AND all visible+arc-gen examples pass.

Outputs:
  reports/dsl_v1_search_results.csv  — every task that had a candidate
  reports/dsl_v1_swap_candidates.csv  — subset that is safe to swap into the anchor
"""
...
def main() -> None:
    ...
    p.add_argument("--tasks", type=str, default="",
                   help="Comma-separated task ids; if empty, samples from [1..400].")
    p.add_argument("--sample-size", type=int, default=100,
                   help="If --tasks empty, randomly sample this many task ids.")
    ...
    p.add_argument("--out-csv", default=str(ROOT / "reports/dsl_v1_search_results.csv"))
    p.add_argument("--swap-csv", default=str(ROOT / "reports/dsl_v1_swap_candidates.csv"))
    p.add_argument("--out-dir", default=str(ROOT / "submissions/handbuilds/dsl_v1"))
    ...
    if args.tasks:
        ...
        task_ids = sorted(rng.sample(range(1, 401), args.sample_size))

    baseline = baseline_lookup(pathlib.Path(args.baseline_csv))

    # Skip tasks already covered by phase4 known DSL hits — we already know
    # they hit. Including them again is fine but adds noise.
    phase4_known = {87, 140, 150, 155, 179, 241, 380, 276, 309, 337}
    ...
    for i, tid in enumerate(task_ids, 1):
        print(f"[{i}/{len(task_ids)}] task{tid:03d}", end=" ", flush=True)
        t0 = time.time()
    ...
    print(
        f"\nsummary: scanned {len(task_ids)} tasks, "
        f"{n_hits} search-hits ({100.0*n_hits/len(task_ids):.1f}%), "
